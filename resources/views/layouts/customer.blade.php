...
<body class="bg-bg text-text font-sans antialiased min-h-screen flex flex-col pb-20 md:pb-0">
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-card border-t border-border py-2 px-6 flex justify-around items-center z-40 md:hidden shadow-lg">
        ...
        <a href="{{ route('customer.ai-scan', ['branch_code' => $branchCode]) }}"
            class="flex flex-col items-center gap-1 {{ request()->routeIs('customer.ai-scan') ? 'text-primary-strong font-bold' : 'text-text-muted hover:text-primary' }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="{{ route('customer.cart', ['branch_code' => $branchCode]) }}"
            ...
            x-data="{ count: {{ array_sum(array_column(session('cart', []), 'quantity')) }} }" @cart-updated.window="count = $event.detail.count">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
            </svg>
            <span x-show="count > 0" x-text="count"
                class="absolute -top-1 right-2 bg-primary text-white t-size1 w-4 h-4 rounded-full flex items-center justify-center font-bold"
        ...
        <a href="{{ route('customer.order.status', ['branch_code' => $branchCode]) }}"
            class="flex flex-col items-center gap-1 {{ request()->routeIs('customer.order.status*') ? 'text-primary-strong font-bold' : 'text-text-muted hover:text-primary' }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
