            ...
            <div class="flex items-center justify-between pb-4 border-b border-border/60 mb-6">
                <div>
                    ...
                    <p class="text-text-muted t-size2 mt-0.5">
                        Perbarui nama peran dan sesuaikan penugasan hak akses modul.
                    </p>
                </div>
            ...
            <form method="POST" action="{{ route('admin.roles.update', $role->id) }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                        {{ in_array($role->name, ['admin', 'cashier', 'kitchen']) ? 'disabled' : '' }}
                        placeholder="Misal: supervisor, manager, dll."
                        class="w-full bg-card border border-border focus:border-primary focus:ring-1 focus:ring-primary rounded-xl px-4 py-3 t-size4 outline-hidden transition {{ in_array($role->name, ['admin', 'cashier', 'kitchen']) ? 'opacity-60 cursor-not-allowed bg-surface-alt' : '' }}">
