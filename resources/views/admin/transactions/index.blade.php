            ...
            <form method="GET" action="{{ route('admin.transactions.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4 pt-2">
                ...
                <div class="space-y-1">
                    ...
                    <select name="method" id="method"
                        ...
                        <option value="cash" {{ $currentMethod === 'cash' ? 'selected' : '' }}>{{ __('Cash') }}
                        </option>
                        <option value="qris" {{ $currentMethod === 'qris' ? 'selected' : '' }}>{{ __('QRIS') }}
                        </option>
                    </select>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="pending" {{ $currentStatus === 'pending' ? 'selected' : '' }}>
                            {{ __('Pending') }}</option>
                        <option value="success" {{ $currentStatus === 'success' ? 'selected' : '' }}>
                            {{ __('Success') }}</option>
                        <option value="failed" {{ $currentStatus === 'failed' ? 'selected' : '' }}>{{ __('Failed') }}
                        </option>
                    </select>
                </div>
                                ...
                                <tr class="hover:bg-surface/30 transition">
                                    ...
                                    <td class="py-4 px-6">
                                        <a href="{{ route('admin.orders.show', $tx->order_id) }}"
                                            class="text-primary hover:text-primary-strong font-bold t-size3 hover:underline">
                                            #{{ $tx->order_id }}
                                        </a>
                                    </td>
                                    <td class="py-4 px-6">
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full t-size1 font-bold border
