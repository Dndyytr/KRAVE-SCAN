<x-app-layout>
    ...
    <div class="space-y-6 anim-fade" x-data="{
        ...
        <div class="bg-card border border-border rounded-2xl p-6 shadow-xs space-y-4">
            ...
            <form method="GET" action="{{ route('admin.activity-logs.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-2">
                ...
                @else
                    <div>
                        <label class="block text-text-muted t-size2 font-semibold mb-1.5">Cabang</label>
                        <input type="text" readonly value="{{ auth()->user()->branch?->name }}"
                            class="w-full bg-surface border border-border rounded-xl px-3 py-2.5 t-size4 outline-hidden text-text-muted cursor-not-allowed">
                    </div>
                @endif
                ...
                <div>
                    ...
                    <input type="date" name="date_from" value="{{ request('date_from') }}"
                        class="w-full bg-card border border-border focus:border-primary focus:ring-1 focus:ring-primary rounded-xl px-3 py-2 t-size4 outline-hidden transition">
                </div>
                                ...
                                <tr class="hover:bg-surface/30 transition">
                                    ...
                                    <td class="py-4 px-6 text-text t-size3">
                                        @if ($log->user)
                                            ...
                                            <span class="block t-size1 text-text-muted">ID: {{ $log->user_id }} -
                                                {{ ucfirst($log->user->role?->name ?? 'Staff') }}</span>
                                        @else
                                    ...
                                    <td class="py-4 px-6 whitespace-nowrap">
                                        @if ($log->action === 'created')
                                            <span
                                                class="bg-success-soft/30 text-success font-bold px-2.5 py-0.5 rounded-full t-size1 border border-success-soft/60">
                                                Created
                                            </span>
                                        @elseif($log->action === 'updated')
                                            <span
                                                class="bg-warning-soft/30 text-warning font-bold px-2.5 py-0.5 rounded-full t-size1 border border-warning-soft/60">
                                                Updated
                                            </span>
                                        @elseif($log->action === 'deleted')
                                            <span
                                                class="bg-danger-soft/30 text-danger font-bold px-2.5 py-0.5 rounded-full t-size1 border border-danger-soft/60">
                                                Deleted
                                            </span>
                                        ...
                                        @elseif($log->action === 'logout')
                                            <span
                                                class="bg-text-muted/10 text-text-muted font-bold px-2.5 py-0.5 rounded-full t-size1 border border-text-muted/20">
                                                Logout
                                            </span>
                                        @else
                                            <span
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        @if ($log->old_values || $log->new_values)
                                            <button @click="openModal({{ json_encode($log) }})"
                                                class="text-accent hover:text-primary font-bold t-size3 transition cursor-pointer">
                                                Inspeksi
                                            </button>
                                        @else
                                            <span class="text-text-muted/40 font-bold t-size3 cursor-not-allowed select-none">Tidak
                                                ada</span>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div x-show="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" style="display: none;"
            ...
            <div class="bg-card border border-border rounded-2xl max-w-3xl w-full p-6 shadow-lg space-y-4" @click.away="showDetailModal = false">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pt-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="bg-surface border border-border rounded-xl p-4 overflow-x-auto">
                            ...
                            <template x-if="!selectedLog || !selectedLog.old_values">
                                <span class="text-text-muted italic t-size3">Tidak ada data sebelumnya (Baru
                                    dibuat).</span>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="bg-surface border border-border rounded-xl p-4 overflow-x-auto">
                            ...
                            <template x-if="!selectedLog || !selectedLog.new_values">
                                <span class="text-text-muted italic t-size3">Tidak ada data sesudahnya
                                    (Dihapus).</span>
                            </template>
                        </div>
                    </div>
                ...
                <div class="flex justify-end pt-2">
                    <button @click="showDetailModal = false"
                        class="bg-surface border border-border text-text hover:bg-border px-5 py-2.5 rounded-xl transition cursor-pointer font-bold t-size4">
                        Tutup
                    </button>
                </div>
            </div>
