        ...
        <div class="bg-card border border-border rounded-2xl p-6 shadow-xs space-y-4">
            ...
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-wrap md:flex-nowrap gap-3 pt-2">
                <div class="relative flex-grow">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau email staf..."
                            ...
                            @foreach ($users as $user)
                                <tr class="hover:bg-surface/30 transition">
                                    ...
                                    <td class="py-4 px-6 font-bold text-text t-size4">
                                        ...
                                        @if ($user->id === auth()->id())
                                            <span
                                                class="ml-1 bg-primary-soft/50 text-accent font-bold px-1.5 py-0.5 rounded t-size1 border border-primary-soft/80">Anda</span>
                                        @endif
                                    </td>
                                    <!-- Email -->
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="flex justify-end gap-3">
                                            ...
                                            @if ($user->id !== auth()->id())
                                                <button @click="confirmDelete('{{ route('admin.users.destroy', $user->id) }}')"
        ...
        <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" style="display: none;"
            ...
            <div class="bg-card border border-border rounded-2xl max-w-md w-full p-6 shadow-lg space-y-4" @click.away="showDeleteModal = false">
                ...
                <p class="text-text-muted t-size3">
                    Apakah Anda yakin ingin menghapus staf ini? Semua data login dan histori aksi yang terkait mungkin
                    tidak dapat diakses lagi. Tindakan ini tidak dapat dibatalkan.
                </p>
