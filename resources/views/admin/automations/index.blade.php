            ...
            <form method="GET" action="{{ route('admin.automations.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
                ...
                <div class="space-y-1">
                    ...
                    <select name="task_name" id="task_name"
                        ...
                        <option value="Generate Receipt" {{ request('task_name') === 'Generate Receipt' ? 'selected' : '' }}>
                            {{ __('Generate Receipt') }}</option>
                        <option value="Low Stock Warning" {{ request('task_name') === 'Low Stock Warning' ? 'selected' : '' }}>
                            {{ __('Low Stock Warning') }}</option>
                        <option value="Auto Cancel Order" {{ request('task_name') === 'Auto Cancel Order' ? 'selected' : '' }}>
                            {{ __('Auto Cancel Order') }}</option>
                        <option value="Aggregate Daily Reports" {{ request('task_name') === 'Aggregate Daily Reports' ? 'selected' : '' }}>
                            {{ __('Aggregate Daily Reports') }}</option>
                    </select>
                </div>
                ...
                <div class="{{ $isSuperAdmin ? 'md:col-span-1' : 'md:col-span-2' }} flex items-end justify-end gap-3 pb-0.5">
                    <button type="submit"
                        class="bg-primary hover:bg-primary-strong text-white font-bold px-6 py-2.5 rounded-xl transition shadow-xs cursor-pointer w-full md:w-auto">
                        {{ __('Filter') }}
                    </button>
                        ...
                        <tbody class="divide-y divide-border t-size3 text-text">
                            @foreach ($logs as $log)
                                <tr class="hover:bg-surface/50 transition">
                                    ...
                                    <td class="py-4 px-6 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full t-size1 font-extrabold uppercase border
                                            ...
                                            @else bg-danger/10 text-danger border-danger/30 @endif">
                                            {{ $log->status }}
                                        </span>
                                    </td>
