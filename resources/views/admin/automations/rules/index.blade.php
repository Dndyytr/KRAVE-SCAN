<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.automations.index') }}"
                    class="px-4 py-2.5 rounded-xl t-size3 font-bold transition {{ request()->routeIs('admin.automations.index') ? 'bg-primary text-white shadow-xs' : 'bg-surface border border-border text-text-muted hover:text-text hover:bg-surface-alt' }}">
                    {{ __('Log Aktivitas') }}
                </a>
                <a href="{{ route('admin.automations.rules.index') }}"
                    class="px-4 py-2.5 rounded-xl t-size3 font-bold transition {{ request()->routeIs('admin.automations.rules.index') ? 'bg-primary text-white shadow-xs' : 'bg-surface border border-border text-text-muted hover:text-text hover:bg-surface-alt' }}">
                    {{ __('Aturan & Pemicu') }}
                </a>
            </div>
        </div>
    ...
    <div class="space-y-6 anim-fade" x-data="rulesManager()">
        ...
        <div class="bg-card border border-border rounded-2xl p-6 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div>
                <a href="{{ route('admin.automations.rules.create') }}"
                    class="bg-primary hover:bg-primary-strong text-white font-bold px-5 py-3 rounded-xl transition shadow-xs flex items-center justify-center gap-2 t-size3 cursor-pointer">
                            ...
                            @foreach ($rules as $rule)
                                <tr class="hover:bg-surface/50 transition">
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="t-size2">
                                            ...
                                            @elseif($rule->condition_type === 'min_order_amount')
                                                <span class="font-bold text-text">{{ __('Min Belanja') }}:</span> <span
                                                    class="text-accent font-bold">Rp{{ number_format($rule->condition_value['min_amount'] ?? 0, 0, ',', '.') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <button @click="toggleRule({{ $rule->id }})"
                                            :class="states[{{ $rule->id }}] ? 'bg-success text-white' :
                                    ...
                                    <td class="py-4 px-6 text-right whitespace-nowrap space-x-2">
                                        <a href="{{ route('admin.automations.rules.edit', $rule->id) }}"
                                            class="inline-block bg-surface border border-border hover:bg-border text-text-muted hover:text-text font-bold px-3 py-1.5 rounded-xl transition t-size2 cursor-pointer shadow-2xs">
                                            {{ __('Edit') }}
                                        </a>
                                        <form method="POST" action="{{ route('admin.automations.rules.destroy', $rule->id) }}" class="inline-block"
                                            onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus aturan otomatisasi ini?') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="bg-danger/10 hover:bg-danger text-danger hover:text-white border border-danger/30 font-bold px-3 py-1.5 rounded-xl transition t-size2 cursor-pointer shadow-2xs">
                                                {{ __('Hapus') }}
                                            </button>
                                        </form>
                                    </td>
