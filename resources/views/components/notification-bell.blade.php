...
setInterval(() => fetchNotifications(), 10000)" class="relative">

    <!-- Bell Button -->
    <button @click="open = !open" @click.outside="open = false"
        class="relative p-2 text-text-muted hover:text-text hover:bg-surface rounded-full transition focus:outline-none cursor-pointer">
        ...
        <template x-if="unreadCount > 0">
            <span class="absolute top-1 right-1 flex h-4 w-4">
                ...
                <span class="relative inline-flex rounded-full h-4 w-4 bg-danger text-[9px] font-bold text-white items-center justify-center"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95"
        ...
        class="absolute right-0 mt-2 w-80 bg-card border border-border rounded-2xl shadow-lg z-50 overflow-hidden" style="display: none;">
        ...
        <div class="px-4 py-3 border-b border-border flex items-center justify-between bg-surface-alt">
            ...
            <button x-show="unreadCount > 0" @click="markAllAsRead()"
                class="text-accent hover:text-accent/80 font-semibold t-size2 transition focus:outline-none cursor-pointer">
                Tandai semua dibaca
            </button>
        </div>
