<x-app-layout>
    ...
    <div class="space-y-8 anim-fade">
        <!-- Welcome Section -->
        <div
            class="bg-gradient-to-br from-primary-soft/30 via-card to-secondary/40 border border-border rounded-2xl p-6 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
            ...
            <div
                class="relative z-10 bg-card/80 backdrop-blur-sm border border-border px-4 py-2 rounded-xl text-text-muted t-size2 font-semibold flex items-center gap-2 shadow-xs">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-card border border-border rounded-2xl p-5 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 group">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div
                        class="w-10 h-10 bg-primary-soft/30 text-accent rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                ...
                <div class="mt-2 flex items-center gap-1.5 t-size1 font-semibold {{ $revDiff >= 0 ? 'text-green-600' : 'text-red-500' }}">
                    ...
                    @endif
                    <span>{{ $yesterdayRevenue > 0 ? abs(round(($revDiff / $yesterdayRevenue) * 100)) . '%' : ($revDiff > 0 ? '∞' : '0%') }}
                        vs kemarin</span>
                </div>
                <!-- Sparkline -->
            ...
            <div class="bg-card border border-border rounded-2xl p-5 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 group">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div
                        class="w-10 h-10 bg-info-soft text-info rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-2 flex items-center gap-1.5 t-size1 font-semibold {{ $ordDiff >= 0 ? 'text-green-600' : 'text-red-500' }}">
                    ...
                    @endif
                    <span>{{ $yesterdayOrdersCount > 0 ? abs(round(($ordDiff / $yesterdayOrdersCount) * 100)) . '%' : ($ordDiff > 0 ? '∞' : '0%') }}
                        vs kemarin</span>
                </div>
                <!-- Sparkline -->
            ...
            <div
                class="bg-card border border-border rounded-2xl p-5 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 group {{ $pendingOrdersCount > 0 ? 'border-l-4 border-l-warning' : '' }}">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-text-muted t-size2 font-bold uppercase tracking-wider">
                        {{ __('Pesanan Pending') }}
                    </span>
                    <div
                        class="w-10 h-10 bg-warning-soft text-warning rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 {{ $pendingOrdersCount > 0 ? 'anim-pulse-soft' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div
                class="bg-card border border-border rounded-2xl p-5 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 group {{ $lowStockCount > 0 ? 'border-l-4 border-l-danger' : '' }}">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-text-muted t-size2 font-bold uppercase tracking-wider">
                        {{ __('Stok Menipis') }}
                    </span>
                    <div
                        class="w-10 h-10 bg-danger-soft text-danger rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 {{ $lowStockCount > 0 ? 'anim-pulse-soft' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="bg-card border border-border rounded-2xl p-6 shadow-xs">
                ...
                <p class="text-text-muted t-size2 mb-4">{{ __('Pendapatan harian berdasarkan pembayaran berhasil') }}
                </p>
                            ...
                            <tbody class="divide-y divide-border">
                                @foreach ($recentOrders as $order)
                                    <tr class="hover:bg-primary-soft/5 transition-colors duration-150">
                                        ...
                                        <td class="py-3 px-4">
                                            <span
                                                class="bg-primary-soft/40 text-accent font-bold px-2 py-0.5 rounded-full t-size1 border border-primary-soft/50">
                                                {{ $order->table_number }}
                                            </span>
                                        </td>
                ...
                @else
                    <div class="space-y-3">
                        @foreach ($lowStockItems as $item)
                            <div
                                class="flex items-center justify-between p-4 bg-danger-soft/60 border border-danger/20 rounded-xl hover:bg-danger-soft transition-colors duration-150">
                                <div class="space-y-1">
                        ...
                        series: [{
                            ...
                            areaStyle: {
                                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                                        offset: 0,
                                        color: 'rgba(232,140,162,0.3)'
                                    },
                                    {
                                        offset: 1,
                                        color: 'rgba(232,140,162,0.02)'
                                    }
                                ])
                            }
                        }],
                    ...
                    ordSpark.setOption({
                        ...
                        series: [{
                            ...
                            areaStyle: {
                                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                                        offset: 0,
                                        color: 'rgba(136,199,232,0.3)'
                                    },
                                    {
                                        offset: 1,
                                        color: 'rgba(136,199,232,0.02)'
                                    }
                                ])
                            }
                        }],
                        ...
                        series: [{
                            ...
                            emphasis: {
                                itemStyle: {
                                    ...
                                    borderWidth: 3,
                                    shadowBlur: 10,
                                    shadowColor: 'rgba(136,199,232,0.4)'
                                }
                            }
                        }]
                    });
