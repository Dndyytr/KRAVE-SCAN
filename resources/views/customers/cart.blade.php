        ...
        <div
            ...
            <aside
                ...
                <div class="mb-8.75 mt-17 flex flex-col items-center">
                    ...
                    <strong class="-mt-2.75 font-brand text-[28px] font-extrabold italic leading-none tracking-[-.055em] text-[#e95578]">Bakso
                        Cinta</strong>
                ...
                <div class="grid grid-cols-[48px_1fr] items-center rounded-[15px] bg-white/80 px-3.5 py-[10px] shadow-[0_5px_20px_rgba(174,111,112,.07)]">
                    ...
                    <span><small class="block t-size1 text-[#706865]">Meja Anda</small><strong
            ...
            <main class="min-w-0 px-3 pb-28 pt-3 bp360:px-4 md:px-7 md:pb-8 md:pt-6 lg:px-8 lg:pb-8 lg:pt-8 lg:h-full lg:overflow-y-auto">
                ...
                <div class="flex items-start justify-between gap-5">
                    <div>
                        ...
                        <h1 class="mt-1 font-heading text-2xl font-extrabold tracking-tight text-text md:text-3xl">
                            Pesanan Anda</h1>
                        <p class="mt-2 t-size1 text-text-muted md:text-xs">Periksa kembali pesanan Anda sebelum
                            checkout.</p>
                    </div>
                ...
                <div x-cloak x-show="message" x-text="message"
                    class="mt-4 rounded-xl border border-danger/30 bg-danger/10 p-3 text-xs font-semibold text-danger">
                </div>
                ...
                <section x-show="isEmpty()" x-cloak class="mt-7 rounded-3xl border border-border bg-white p-10 text-center shadow-sm">
                    ...
                    <a href="{{ route('customer.menu', ['branch_code' => $branch_code, 'table_number' => $table]) }}"
                        class="mt-5 inline-flex rounded-full bg-primary px-6 py-3 text-xs font-bold text-white">Lihat
                        Menu</a>
                </section>
                ...
                <div x-show="!isEmpty()" x-cloak>
                    <section class="mt-6 overflow-hidden rounded-2xl border border-[#f0e5e2] bg-white shadow-[0_6px_22px_rgba(113,74,69,.08)]">
                        <div
                            class="hidden grid-cols-[minmax(0,1.8fr)_110px_120px_110px_42px] gap-3 border-b border-border px-5 py-4 t-size1 font-bold text-text-muted lg:grid">
                                    ...
                                    <div class="col-span-2 flex min-w-0 gap-3 lg:col-span-1 lg:items-center">
                                        <div
                                            ...
                                            <template x-if="item.image_path"><img
                                                    :src="item.image_path.startsWith('http') ? item.image_path : '/' + item
                                                        .image_path"
                                                    :alt="item.name" class="size-full object-cover"></template>
                                        ...
                                        <div class="min-w-0 py-1">
                                            ...
                                            <p class="mt-1 text-[9px] text-text-muted lg:hidden" x-text="'Rp ' + Number(item.price).toLocaleString('id-ID')">
                                                ...
                                                <p class="mt-2 line-clamp-2 text-[8px] font-semibold text-primary">
                                                    <span>Catatan: </span><span x-text="item.note"></span>
                                                </p>
                                            </template>
                                        </div>
                                    </div>
                                    <div class="col-span-2 flex items-center justify-between gap-2 lg:contents">
                                        <span class="hidden t-size1 font-bold lg:block" x-text="'Rp ' + Number(item.price).toLocaleString('id-ID')"></span>
                                        <div class="grid grid-cols-[32px_38px_32px] overflow-hidden rounded-[10px] border border-[#f3ccd4] bg-white">
                                            <button type="button" :disabled="busy" @click="updateQty(item.cart_key,item.quantity-1)"
                                                class="grid h-8 place-items-center text-base text-primary disabled:opacity-40"
                                                aria-label="Kurangi jumlah">−</button><span
                                                class="grid h-8 place-items-center border-x border-[#f5e1e5] t-size1 font-bold"
                                        ...
                                        <strong class="t-size1 font-extrabold text-primary lg:text-right"
                    ...
                    <div class="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
                        <div class="space-y-5">
                            <section class="rounded-2xl border border-[#f0e5e2] bg-white p-4 shadow-[0_5px_18px_rgba(113,74,69,.06)] md:p-5">
                                <div class="flex items-center gap-3 border-b border-border pb-3"><span
                                        ...
                                        <p class="mt-0.5 text-[9px] text-text-muted">Data Anda digunakan untuk
                                            identifikasi pesanan.</p>
                                    </span></div>
                                <div class="mt-4 grid gap-4 md:grid-cols-2">
                                    <label class="block t-size1 font-semibold text-text-muted">Nama Lengkap<input type="text" name="customer_name"
                                    ...
                                    <label class="block t-size1 font-semibold text-text-muted">No. WhatsApp /
                                        Email<input type="text" name="customer_contact" form="checkout-form" required
                                            value="{{ old('customer_contact') }}" placeholder="081234567890 / email"
                                            class="mt-2 h-11 w-full rounded-xl border border-border bg-[#fffaf8] px-3 text-xs text-text outline-none transition duration-200 focus:border-primary focus:ring-2 focus:ring-primary/15"></label>
                                </div>
                                @error('customer_name')
                                    <p class="mt-2 t-size1 font-semibold text-danger">{{ $message }}</p>
                                @enderror
                                @error('customer_contact')
                                    <p class="mt-2 t-size1 font-semibold text-danger">{{ $message }}</p>
                                @enderror
                            </section>
                            <div class="flex items-center gap-3 rounded-2xl bg-[linear-gradient(100deg,#fff5f2,#fde8e4)] p-4 t-size1 text-text-muted">
                                <span class="grid size-10 shrink-0 place-items-center rounded-full bg-white text-primary"><svg class="size-5"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                                        <path d="M12 3 5 6v5c0 4.8 2.8 8.4 7 10 4.2-1.6 7-5.2 7-10V6l-7-3Z" />
                                        <path d="m9.5 12 1.6 1.6 3.6-4" />
                                    </svg></span><span><strong class="block text-xs text-text">Pesanan Anda
                                        aman</strong>Data pesanan dilindungi dan hanya
                                    digunakan untuk proses pemesanan.</span>
                            </div>
                            <a href="{{ route('customer.menu', ['branch_code' => $branch_code, 'table_number' => $table]) }}"
                                class="hidden h-11 w-[155px] items-center justify-center gap-2 rounded-[10px] border border-primary t-size1 font-bold text-primary transition duration-200 hover:bg-primary hover:text-white lg:flex">←
                                Lanjut Belanja</a>
                        </div>
                        ...
                        <aside class="h-fit rounded-2xl border border-[#f0e5e2] bg-white p-5 shadow-[0_7px_22px_rgba(113,74,69,.08)]">
                            ...
                            <div class="mt-5 space-y-3 t-size1 text-text-muted">
                                <div class="flex justify-between gap-4"><span>Subtotal (<b x-text="cartCount"></b>
                                        item)</span><span class="font-semibold text-text" x-text="'Rp ' + cartTotal.toLocaleString('id-ID')"></span></div>
                                ...
                                <div class="flex justify-between gap-4"><span>Cabang</span><span
                                        class="max-w-[170px] truncate font-semibold text-text">{{ $branch }}</span>
                                </div>
                            </div>
                    ...
                    <a href="{{ route('customer.menu', ['branch_code' => $branch_code, 'table_number' => $table]) }}"
                        class="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-[10px] border border-primary t-size1 font-bold text-primary lg:hidden">←
                        Lanjut Belanja</a>
                </div>
            </main>
        </div>
