        ...
        <div
            ...
            <main class="min-w-0 px-3 pb-28 pt-3 bp360:px-4 md:px-7 md:pb-8 md:pt-6 lg:px-8 lg:pb-7 lg:pt-8 lg:h-full lg:overflow-y-auto">
                <header class="mb-5 flex items-center justify-between lg:hidden"><a
                        href="{{ $order ? route('customer.order.status', ['branch_code' => $branch_code, 'order' => $order->id]) : route('customer.menu', ['branch_code' => $branch_code, 'table_number' => $table]) }}"
                        class="grid size-9 place-items-center rounded-full border border-border bg-white" aria-label="Kembali">←</a>
                    <h1 class="font-heading text-lg font-extrabold">Pembayaran ❧</h1><span
                ...
                @else
                    <div class="mt-6 grid gap-5 md:grid-cols-[minmax(0,1.25fr)_minmax(280px,.75fr)]" x-data="{ payMethod: 'qris' }">
                        <section class="min-w-0 rounded-2xl border border-[#f0e5e2] bg-white p-3 shadow-[0_7px_22px_rgba(113,74,69,.08)] bp360:p-4 md:p-5">
                            ...
                            <div x-show="payMethod==='qris'" x-transition class="mt-4 flex flex-col items-center">
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        <svg class="size-full text-text" viewBox="0 0 100 100" fill="currentColor" aria-label="Kode QRIS simulasi">
                                ...
                                <form action="{{ route('customer.pay-qris', ['branch_code' => $branch_code, 'order' => $order->id]) }}" method="POST"
                                    class="mt-4 w-full">@csrf<button type="submit"
                                        class="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[linear-gradient(100deg,#f63f6f,#fa557a)] text-xs font-bold text-white shadow-[0_8px_18px_rgba(242,77,115,.18)] transition duration-200 hover:-translate-y-0.5 hover:shadow-lg"><svg
                                            class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="m8 12 3 3 5-6" />
                                            <circle cx="12" cy="12" r="9" />
                                        </svg>Konfirmasi Pembayaran QRIS</button></form>
                            </div>

                            <div x-cloak x-show="payMethod==='cashier'" x-transition
                                class="mt-4 rounded-2xl border border-[#f2dfe0] bg-[linear-gradient(145deg,#fff8f5,#fff)] p-6 text-center md:min-h-[420px] md:flex-col md:items-center md:justify-center">
                                <span class="mx-auto grid size-20 place-items-center rounded-full bg-primary-soft/25 text-primary"><svg class="size-10"
                        ...
                        <aside class="space-y-4">
                            <section class="rounded-2xl border border-[#f0e5e2] bg-white p-4 shadow-[0_7px_22px_rgba(113,74,69,.08)] md:p-5">
                            ...
                            <div
                                ...
                                <span class="grid size-9 shrink-0 place-items-center rounded-full bg-white text-warning"><svg class="size-5"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                                        <path
